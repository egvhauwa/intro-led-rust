...
#[doc = "Field `PMSTAT` reader - When debug is enabled, the PMSTAT will not update to STOP or VLPS When a PSTOP mode is enabled, the PMSTAT will not update to STOP or VLPS"]
pub type PMSTAT_R = crate::FieldReader<u8, u8>;
impl R {
    #[doc = "Bits 0:7 - When debug is enabled, the PMSTAT will not update to STOP or VLPS When a PSTOP mode is enabled, the PMSTAT will not update to STOP or VLPS"]
    #[inline(always)]
    pub fn pmstat(&self) -> PMSTAT_R {
        PMSTAT_R::new(self.bits)
    }
}
